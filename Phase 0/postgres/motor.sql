...
-- Melihat kolom name dan founded_at --
SELECT name, founded_at
	FROM crunchbase_companies_clean_data;

-- Mengurutkan berdasarkan founded at --
SELECT name, founded_at
	FROM crunchbase_companies_clean_data
...
-- Extract value dari column founded_at --
SELECT
	name,
	founded_at,
...
-- Mencari perusahaan yang didirikan pada 2010 --
SELECT
	name,
	founded_at,
	EXTRACT(year from founded_at) AS tahun,
...
-- Mencari tahu lama berdirinya suatu perusahaan --
SELECT 
	name, 
	founded_at,
	CURRENT_DATE AS date,
...
SELECT
	name,
	founded_at,
	COALESCE(founded_at, '01-01-2000')
FROM crunchbase_companies_clean_data;
...
SELECT
	TRIM(leading 'M' from 'MMMMMMMMMmMADAM') as result_trimming,
	TRIM(trailing 'M' from 'MADAM') as result_trailing,
	TRIM(both 'M' from 'MADAM') as result_both;

-- Menghapus karakter "WO" pada column bike_number --
SELECT
	bike_number,
	TRIM(LEADING 'WO' from bike_number) AS result_trim,
	LENGTH(bike_number) AS panjang awal,
	LENGTH(TRIM(LEADING 'WO' from bike_number)) AS panjang_setelah_trim
FROM dc_bikeshare_q1_2012;
